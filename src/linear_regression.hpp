...
class LinearRegression {
 private:
  ...
  Derived GradientDescentOnce(
      const Eigen::Matrix<Derived, Eigen::Dynamic, Eigen::Dynamic> &X,
      const Eigen::Matrix<Derived, Eigen::Dynamic, 1> &Y,
      Eigen::Matrix<Derived, Eigen::Dynamic, 1> &new_param,
 ...
 public:
  ...
  Derived ComputeCost(
      const Eigen::Matrix<Derived, Eigen::Dynamic, Eigen::Dynamic> &,
      const Eigen::Matrix<Derived, Eigen::Dynamic, 1> &, double lambda = 0);
...
/*
  Constructor of the LinearRegression class: set reugularizing_all_ to false.
...
Derived LinearRegression<Derived>::NormalEquation(
  ...
  if (fit_intercept_) {  // fit intercept option is on
    ...
    X_plus << Eigen::Matrix<Derived, Eigen::Dynamic, 1>::Ones(Y.rows()), X;
    parameters_ << (X_plus.transpose() * X_plus).inverse() * X_plus.transpose() * Y;
...
double LinearRegression<Derived>::CheckLearningRate(
  ...
  do {
    double cost_before_tuning = GradientDescent(X, Y, 1, learning_rate, lambda);

    // Compute the cost
    double cost_after_tuning = GradientDescent(X, Y, 1, learning_rate, lambda);
...
/*
  Use gradient descent once to compute the cost after one tune
...
*/
template <class Derived>
Derived LinearRegression<Derived>::GradientDescentOnce(
    const Eigen::Matrix<Derived, Eigen::Dynamic, Eigen::Dynamic> &X,
    const Eigen::Matrix<Derived, Eigen::Dynamic, 1> &Y,
    Eigen::Matrix<Derived, Eigen::Dynamic, 1> &new_param, double learning_rate,
    double lambda) {
  if (!usable_) return -1;

  Eigen::Map<Eigen::Matrix<Derived, Eigen::Dynamic, 1>, 1,
             Eigen::InnerStride<1>>
      param(new_param.data() + 1, new_param.rows() - 1,
            Eigen::InnerStride<1>());

  Eigen::Matrix<Derived, Eigen::Dynamic, Eigen::Dynamic> X_plus(X.rows(),
                                                                X.cols() + 1);
  ...
  if (lambda == 0) {
    if (fit_intercept_) {
      new_param =
          new_param - learning_rate / batch_size *
                          (X_plus.transpose() * (X_plus * new_param - Y));

    } else {
      param = param - learning_rate / batch_size *
                          (X.transpose() * (X_plus * new_param - Y));
    }
  } else if (fit_intercept_) {  // Regularize theta_0
    new_param = new_param -
                learning_rate / batch_size *
